...
TOOL_DESCRIPTIONS_V3 = {
    ...
    "notion": (
        "Gestion directe de TOUTES les databases Notion : Goals, Tasks, Decisions, "
        "Habits, Sports, Ideas, Business Tracker, Error Tracker. "
        "Lire, creer, modifier, chercher. Utilise pour toute operation CRUD."
    ),
}
...
9. notion : Gestion directe de TOUTES les databases Notion (8 databases : Goals, Tasks, Decisions, Habits, Sports, Ideas, Business, Errors).
   Lire, creer, modifier, chercher. Utilise pour toute operation CRUD sur Notion.

== RACCOURCIS ==
/strategy→strategy, /score→prioritize, /decision→decision, /bilan→progress, /prospect→prospect, /search→search, /scrape→scrape, /knowledge→knowledge, /notion→notion
...
# ════════════════════════════════════════════════════════════════════
# Sub: Notion Agent — Couche 3b (generic Notion CRUD via Claude)
# ════════════════════════════════════════════════════════════════════

NOTION_AGENT_SYSTEM_PROMPT = """Tu es un agent Notion specialise. Tu recois une demande en langage naturel et tu generes les parametres d'appel API Notion correspondants.

== DATABASES CONNUES (8) ==

1. Goals (bc88ee5f-f09b-4f45-adb9-faae179aa276)
   Proprietes :
   - Name (title) : nom de l'objectif
   - Status (select) : "💭 Not Started", "🔥 In Progress", "✅ Achieved", "⏸️ On Hold", "❌ Abandoned"
   - Priority (select) : "🔴 Critical", "🟡 High", "🟢 Medium", "⚪ Low"
   - Category (select) : "🏃 Health", "💰 Wealth", "❤️ Relationships", "🧠 Personal Growth", "💼 Business", "🎨 Creative"
   - Progress % (number, format percent, 0 a 1 : 0.3 = 30%)
   - Impact Score (number, 1-10)
   - Effort Score (number, 1-10)
   - Revenue Potential (number, euro)
   - Urgency (select) : "Urgent", "Not Urgent"
   - Strategic Score (formula, LECTURE SEULE)
   - Target Date (date)
   - Type (select) : "🌟 Life Goal", "📅 Annual Goal", "🗓️ Quarterly Goal", "📆 Monthly Goal"
   - Success Criteria (rich_text)
   - Why (rich_text)
   - XP Reward (number), Gold Reward (number)

2. Tasks (305da200-b2d6-8145-bc16-eaee02925a14)
   Nom reel : "Projects & Tasks (Quests)"
   Proprietes :
   - Name (title)
   - Status (STATUS, PAS select) : "Backlog", "Ready To Start", "In Progress", "Blocked", "Complete", "Archive"
   - Category (select) : "💼 Business", "🤖 Automatisation", "🏠 Perso", "📝 Notion", "🎨 Creative"
   - Type (select) : "Project", "Task"
   - Difficulty (select) : "1 - Easy", "2 - Moderate", "3 - Hard"
   - Due Date (date)
   - Description (rich_text)
   - Goal (relation vers Goals)
   - Revenue Impact (select) : "💰 Direct", "📈 Indirect", "❌ None"
   - Upstream (relation self, parent), Downstream (relation self, enfants)
   ATTENTION : Status = type "status". Filtre: {"status": {"equals": "..."}}, Ecriture: {"status": {"name": "..."}}

3. Decision Log (ced391a8-17ee-4115-9e48-9f6ecab02a93)
   Proprietes :
   - Decision (title, PAS "Name")
   - Date (date)
   - Impact (select) : "Positif", "Neutre", "Negatif", "En attente"
   - Context (rich_text), Option choisie (rich_text), Options envisagees (rich_text)
   - Pourquoi (rich_text), Resultat attendu (rich_text), Resultat reel (rich_text), Learnings (rich_text)
   - Goal lie (relation vers Goals)

4. Habits (305da200-b2d6-8139-b19f-d2a0d46cf7e6)
   Nom reel : "Habits Tracker (Recurring Quests)"
   Proprietes :
   - Name (title)
   - Frequency (select) : "0 - Anytime", "1 - Daily", "7 - Weekly", "14 - Bi-Weekly", "30 - Monthly"
   - Difficulty (select) : "1 - Easy", "2 - Moderate", "3 - Hard"
   - Duration (select) : "0.5 🍅 - 15 Mins", "1.0 🍅 - 30 Mins", "1.5 🍅 - 45 Mins", "2.0 🍅 - 1.0 Hours"
   - Type (multi_select) : "Morning", "Mid-Day", "Evening", "Bedtime"
   - Last Completed (date), Streak Days (number), Completed This Month (number), Total This Month (number)
   - Goal (relation vers Goals), Related Goal (relation vers Goals)
   - Description (rich_text)
   - Do Date (formula, LECTURE SEULE), Current Streak (formula), Success Rate % (formula), XP (formula), Gold (formula)

5. Sports (2126d8b4-d7f3-4e41-98ae-4ccb8f959d38)
   Nom reel : "🏋️ Sports Tracker"
   Proprietes :
   - Session (title)
   - Type (select) : "🏃 Cardio", "💪 Musculation", "🧘 Stretching", "⚽ Sport collectif", "🚶 Marche", "🏊 Natation", "🚴 Velo"
   - Date (date)
   - Duration (min) (number)
   - Calories Burned (number)
   - Weight (kg) (number)
   - Feeling (select) : "💪 Excellent", "😊 Good", "😐 Average", "😩 Tired", "🤕 Pain"
   - Notes (rich_text)
   - Related Goal (relation vers Goals)

6. Ideas (4e27976b-b626-4f06-9a4b-3ad4e32c4c19)
   Nom reel : "💡 Ideas Bank"
   Proprietes :
   - Name (title)
   - Status (select) : "💭 New", "🔍 Evaluating", "✅ Approved", "❌ Rejected", "🚀 Converted"
   - Category (select) : "💼 Business", "🤖 Automatisation", "🎥 Content", "📝 Notion", "🎮 Personal", "💡 Other"
   - Priority (select) : "🔴 High", "🟡 Medium", "🟢 Low"
   - Impact (select) : "🟢 Low", "🟡 Medium", "🔴 High", "💫 Game Changer"
   - Effort (select) : "⚡ Quick Win", "🛠️ Medium", "🏗️ Big Project"
   - Notes (rich_text), Source (rich_text)
   - Related Goal (relation vers Goals)

7. Business (9995fa09-4414-4f52-bb1e-6c37cdd98166)
   Nom reel : "💰 Business Tracker"
   Proprietes :
   - Account Name (title)
   - Platform (select) : "TikTok", "YouTube", "Instagram", "Twitter/X", "Pinterest", "Freelance", "Digital Product", "Other"
   - Status (select) : "🌱 Starting", "📈 Growing", "🚀 Scaling", "💰 Monetized", "⏸️ Paused", "❌ Abandoned"
   - Niche (select) : "💬 Reddit Stories", "🚔 Police/Dashcam", "🌙 Sleep/Ambient", "💪 Motivation", "😱 Horror/Creepy", "💡 Facts/Education", "🎨 Other"
   - Monetization (multi_select) : "AdSense", "Creator Fund", "Sponsorships", "Affiliate", "Products", "Not Yet"
   - Automation Level (select) : "🟢 Full Auto", "🟡 Semi Auto", "🔴 Manual"
   - Subscribers (number), Monthly Views (number), Monthly Revenue (€) (number), RPM (€) (number), Posts/Week (number)
   - Account URL (url), Notes (rich_text)
   - Related Goal (relation vers Goals), Related Project (relation vers Tasks)

8. Errors (b6f95d07-ff4a-43d8-9026-1855e8b13c8e)
   Nom reel : "🐛 Error Tracker"
   Proprietes :
   - Error (title)
   - Status (select) : "🆕 New", "🔍 Investigating", "✅ Fixed", "🚫 Won't Fix", "🔄 Recurring"
   - Severity (select) : "🔴 Critical", "🟡 Warning", "🟢 Info"
   - Error Type (select) : "🔌 API Error", "⏱️ Timeout", "🔐 Auth Error", "📊 Data Error", "🧠 Logic Error", "❓ Unknown"
   - Source Type (select) : "n8n Workflow", "Python Script", "Flask API", "Notion API", "External API", "Other"
   - Error Message (rich_text), Stack Trace (rich_text), Solution (rich_text)
   - Source Name (rich_text), Workflow ID (rich_text)
   - Date (date), Occurrences (number), Auto-fixed (checkbox)
   - Related Task (relation vers Tasks)

9. Activity Log (305da200-b2d6-819f-915f-d35f51386aa8)
   Nom reel : "Activity Log"
   Collection : 305da200-b2d6-8116-8039-000b9a9d9070
   Hub central de la gamification — chaque action completee (quest, habit) genere une entree ici.
   Proprietes :
   - Name (title)
   - Date (date)
   - Quests (relation vers Tasks), Habits (relation vers Habits), Bad Habits (relation), Items (relation)
   - Daily Summary (relation vers Daily Summary), Leaderboard (relation vers Leaderboard)
   - XP (formula), Gold (formula), HP (formula), Type (formula), Details (formula), At A Glance (formula)
   ATTENTION : toutes les proprietes calculees (XP, Gold, etc.) sont des FORMULAS en LECTURE SEULE.

10. Leaderboard (305da200-b2d6-81e9-b38b-fc5d1d6a75be)
    Nom reel : "Leaderboard"
    Collection : 305da200-b2d6-81f1-8145-000b6fd7b000
    Profil joueur — Level, stats globales calculees depuis Activity Log.
    Proprietes :
    - Name (title)
    - Log (relation vers Activity Log)
    - Level (formula), Gold (formula), Habits (formula), Health (formula), At A Glance (formula)
    ATTENTION : toutes sauf Name et Log sont des FORMULAS en LECTURE SEULE.

11. Daily Summary (8559b19c-86a5-4034-bc4d-ea45459ef6bd)
    Nom reel : "Daily Summary"
    Collection : 6613764e-a18d-4cf5-8c66-d072afb309b8
    Resume quotidien — XP, Gold, habits du jour, agreges depuis Activity Log.
    Proprietes :
    - Day (title)
    - Date (date)
    - Activities (relation vers Activity Log)
    - Activity Count (rollup, LECTURE SEULE), Habits Completed (rollup), Total Gold (rollup), Total XP (rollup)
    - Day of Week (formula, LECTURE SEULE)

== RACCOURCIS NOMS ==
"goals/objectifs" → Goals | "tasks/taches/projets/quests" → Tasks | "decisions" → Decision Log
"habits/habitudes" → Habits | "sport/sports/training" → Sports | "ideas/idees" → Ideas
"business/comptes/revenue" → Business | "errors/erreurs/bugs" → Errors
"log/activity/activites" → Activity Log | "leaderboard/level/stats" → Leaderboard | "daily/resume/summary" → Daily Summary

== OPERATIONS SUPPORTEES ==
1. query — Lister/filtrer des pages dans une database
2. create — Creer une page dans une database
3. update — Modifier les proprietes d'une page (necessite page_id)
4. get — Recuperer une page par ID
5. search — Recherche full-text dans tout le workspace
6. list_schemas — Retourne les schemas des 11 databases (pas d'appel API)

== FORMAT DE SORTIE ==
Tu DOIS repondre UNIQUEMENT avec un bloc JSON valide, sans texte avant ou apres.

Pour query :
{"operation":"query","database_id":"<db_id>","method":"POST","endpoint":"/v1/databases/<db_id>/query","body":{"filter":{...},"sorts":[...],"page_size":100},"summary":"<desc>"}

Pour create :
{"operation":"create","method":"POST","endpoint":"/v1/pages","body":{"parent":{"database_id":"<db_id>"},"properties":{...}},"summary":"<desc>"}

Pour update :
{"operation":"update","method":"PATCH","endpoint":"/v1/pages/<page_id>","body":{"properties":{...}},"summary":"<desc>"}

Pour get :
{"operation":"get","method":"GET","endpoint":"/v1/pages/<page_id>","body":{},"summary":"<desc>"}

Pour search :
{"operation":"search","method":"POST","endpoint":"/v1/search","body":{"query":"<terme>","page_size":10},"summary":"<desc>"}

Pour list_schemas :
{"operation":"list_schemas","method":null,"endpoint":null,"body":null,"summary":"8 databases : Goals, Tasks, Decisions, Habits, Sports, Ideas, Business, Errors."}

== TYPES DE PROPRIETES NOTION API ==
- title: {"title": [{"text": {"content": "value"}}]}
- rich_text: {"rich_text": [{"text": {"content": "value"}}]}
- select: {"select": {"name": "value"}}
- multi_select: {"multi_select": [{"name": "val1"}, {"name": "val2"}]}
- status: {"status": {"name": "value"}}  (UNIQUEMENT Tasks.Status)
- number: {"number": 42}
...
- multi_select contains: {"property": "Type", "multi_select": {"contains": "Morning"}}
- number: {"property": "Progress %", "number": {"greater_than": 0.5}}
- title/rich_text: {"property": "Name", "title": {"contains": "texte"}}
- date: {"property": "Date", "date": {"after": "2026-01-01"}}
- checkbox: {"property": "Auto-fixed", "checkbox": {"equals": true}}

== REGLES ==
- Pour "en cours" sur Goals → filtre Status = "🔥 In Progress"
- Pour "en cours" sur Tasks → filtre Status = "In Progress" (type status)
- N'inclus PAS les formules (Strategic Score, XP, Gold, At A Glance, Do Date, Current Streak, etc.)
- Pour Progress % : 0.3 = 30%, 0.5 = 50%, 1.0 = 100%
- Pour creer une session sport, le titre est "Session" (PAS "Name")
- Pour Decision Log, le titre est "Decision" (PAS "Name")
- Reponds TOUJOURS en JSON, jamais en texte libre"""


def http_request_notion_node(name, pos):
    """Dynamic HTTP Request node for Notion API calls.

    Uses n8n's Predefined Credential Type for Notion (notionApi),
    which automatically injects the Bearer token.
    Notion-Version header must be added manually.
    """
    return {
        "parameters": {
            "method": "={{ $json.method }}",
...
def if_node_needs_api(name, pos):
    """IF node: check if operation requires an API call (not list_schemas)."""
    return {
        "parameters": {
            "conditions": {
...
def create_sub_notion_agent():
    """Sub-workflow: Notion Agent — generic Notion CRUD via Claude + HTTP Request."""
    nodes = []
    connections = {}

    # 1. Trigger (called by Manager via toolWorkflow)
    nodes.append(execute_workflow_trigger_node("Trigger", [0, 0]))
    ...
    # 3. Claude Notion Agent (Anthropic node — interprets query, generates API params)
    nodes.append(anthropic_node(
        "Claude Notion Agent",
    ...
    connect(connections, "Extract Query", "Claude Notion Agent")

    # 4. Build Request (parse Claude's JSON output into method/url/body)
    nodes.append(code_node("Build Request", """
const raw = $json.content?.[0]?.text || $json.output || $json.text || '';
...
""", [900, 0]))
    connect(connections, "Claude Notion Agent", "Build Request")

    # 5. IF Needs API Call (skip HTTP for list_schemas and errors)
    nodes.append(if_node_needs_api("IF Needs API Call", [1200, 0]))
    connect(connections, "Build Request", "IF Needs API Call")

    # 6. HTTP Request Notion (dynamic — true branch)
    nodes.append(http_request_notion_node("HTTP Request Notion", [1500, -100]))
    connect(connections, "IF Needs API Call", "HTTP Request Notion", from_output=0)

    # 7a. Format & Return (API result path)
    nodes.append(code_node("Format API Result", """
...
return [{json: {response}}];
""", [1800, -100]))
    connect(connections, "HTTP Request Notion", "Format API Result")

    # 7b. Format & Return (no-API path — list_schemas or error)
    nodes.append(code_node("Format Direct Result", """
const data = $json;
if (data.error) {
...
""", [1500, 200]))
    connect(connections, "IF Needs API Call", "Format Direct Result", from_output=1)
...
""", [1200, 400]))
    ...
    tools = [
        ...
        ("Tool: notion",     "notion",     sub_workflow_ids.get("notion"),     [2700, 500]),
    ]
...
CURRENT_MANAGER_V3_ID = "614eolE935GVV5sT"
CURRENT_JINA_SCRAPER_ID = "gNAlXq3Jx5RDRpvu"
CURRENT_KNOWLEDGE_ID = "Z5SkxVoXLGQERTHO"
CURRENT_NOTION_AGENT_ID = "3dYkcNR1hikb45hR"
...
def main_notion_agent():
    """Deploy Notion Agent sub-workflow and update Manager Agent v3 to include it."""
    print("=" * 60)
    print("NOTION AGENT — Deploy & Integrate with Manager v3")
    print("=" * 60)

    created = []

    # Step 1: Create Notion Agent sub-workflow
    print("\n[Step 1] Creating Notion Agent sub-workflow...")
    notion_data = create_sub_notion_agent()
    notion_id, notion_name = create_workflow(notion_data)
    if notion_id:
        print(f"  OK: id={notion_id}, name={notion_name}")
        created.append((notion_id, notion_name))
    else:
        print("  FAILED to create Notion Agent!")
        return created
    time.sleep(1)

    # Step 2: Activate Notion Agent
    print(f"\n[Step 2] Activating Notion Agent ({notion_id})...")
    if activate_workflow(notion_id):
        print("  OK: Notion Agent activated!")
    else:
        print("  WARNING: Could not activate Notion Agent via API")

    # Step 3: Deactivate current Manager v3
    print(f"\n[Step 3] Deactivating Manager v3 ({CURRENT_MANAGER_V3_ID})...")
    deactivate_workflow(CURRENT_MANAGER_V3_ID)
    time.sleep(2)

    # Step 4: Delete current Manager v3
    print(f"[Step 4] Deleting Manager v3 ({CURRENT_MANAGER_V3_ID})...")
    if delete_workflow(CURRENT_MANAGER_V3_ID):
        print("  OK: Manager v3 deleted")
    else:
        print("  WARNING: Could not delete (may already be gone)")
    time.sleep(1)

    # Step 5: Find Postgres credential (for memory)
    print("\n[Step 5] Searching for Postgres credential...")
    pg_cred = find_postgres_credential()
    if pg_cred:
        print(f"  Found: {pg_cred['postgres']['id']} ({pg_cred['postgres']['name']})")
    else:
    ...
    # Step 6: Recreate Manager v3 with all tools including notion
    sub_workflow_ids = {
        "strategy": CURRENT_STRATEGY_ID,
        "prioritize": EXISTING_PRIORITIZER_ID,
        "decision": EXISTING_DECISION_LOGGER_ID,
    ...
    # Include scraper and knowledge if they exist
    if CURRENT_JINA_SCRAPER_ID:
        sub_workflow_ids["scrape"] = CURRENT_JINA_SCRAPER_ID
    if CURRENT_KNOWLEDGE_ID:
        sub_workflow_ids["knowledge"] = CURRENT_KNOWLEDGE_ID

    print(f"\n[Step 6] Creating Manager v3 with Notion tool...")
    tools_list = ', '.join(sub_workflow_ids.keys())
    print(f"  Tools ({len(sub_workflow_ids)}): {tools_list}")
    ...
    # Step 7: Activate Manager
    print(f"\n[Step 7] Activating Manager v3 ({manager_id})...")
    time.sleep(2)
    if activate_workflow(manager_id):
        print("  OK: Manager v3 activated!")
    else:
    ...
    # Summary
    print(f"\n{'='*60}")
    print("NOTION AGENT — Deployment Summary")
    print(f"{'='*60}")
    print(f"\nNotion Agent:  {notion_id}")
    print(f"Manager v3:    {manager_id}")
    memory_type = "Postgres (persistent)" if pg_cred else "Window Buffer (40 msgs)"
    print(f"Memory:        {memory_type}")
    print(f"Tools ({len(sub_workflow_ids)}): {tools_list}")
    print(f"\nDeleted: previous Manager v3 ({CURRENT_MANAGER_V3_ID})")
    print(f"\nTest via Telegram: /notion liste mes goals en cours")

    return created


def main():
    if "--notion-agent" in sys.argv:
        return main_notion_agent()
    elif "--upgrade-v3" in sys.argv:
        return upgrade_v3()
